...
pub mod mon_ord {
    ...
    pub fn lex<const N: usize>(a: MDeg<N>, b: MDeg<N>) -> Ordering {
        a.0.cmp(&b.0)
    }

    /// reverse lexicographic ordering on multidegrees
    pub fn revlex<const N: usize>(a: MDeg<N>, b: MDeg<N>) -> Ordering {
    ...
    pub fn grad<const N: usize>(a: MDeg<N>, b: MDeg<N>) -> Ordering {
    ...
    pub fn grlex<const N: usize>(a: MDeg<N>, b: MDeg<N>) -> Ordering {
        match grad(a, b) {
    ...
    pub fn grevlex<const N: usize>(a: MDeg<N>, b: MDeg<N>) -> Ordering {
        match grad(a, b) {
...
impl<F: Field, const N: usize> Term<F, N> {
    ...
    pub fn eval(&self, x: &[F; N]) -> F {
        x.iter()
            .zip(self.mdeg.degs())
            .map(|(x, d)| x.powi32(*d))
            .fold(self.coef, |acc, x| acc * x) // should impl Product for Field
    }
}
...
impl<F: Field, const N: usize> MulAssign<Term<F, N>> for Polynomial<F, N> {
    fn mul_assign(&mut self, rhs: Term<F, N>) {
...
impl<F: Field, const N: usize> Mul<Term<F, N>> for Polynomial<F, N> {
    ...
    fn mul(mut self, rhs: Term<F, N>) -> Self::Output {
        self *= rhs;
...
impl<F: Field, const N: usize> Mul<Term<F, N>> for &Polynomial<F, N> {
    ...
    fn mul(self, rhs: Term<F, N>) -> Self::Output {
        let mut out = self.clone();
